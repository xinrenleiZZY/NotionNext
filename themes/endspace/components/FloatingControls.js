...
const FloatingControls = ({ toc, ...props }) => {
    ...
    >
      <div
        className={`w-full h-full rounded-full flex items-center justify-center transition-colors duration-200 ${
          active
            ? 'bg-[var(--endspace-accent-yellow)]'
            : 'bg-transparent group-hover:bg-[var(--endspace-accent-yellow)]'
        }`}
      >
        {showPercent ? (
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className='flex items-center justify-between px-4 py-3 border-b border-[var(--endspace-border-base)] bg-[var(--endspace-bg-secondary)] shrinking-0'>
          <h3 className='font-bold text-sm uppercase flex items-center gap-2 text-[var(--endspace-text-primary)]'>
            {activeTab === 'toc' ? (
              <>
                <IconListTree
                  size={16}
                  className='text-[var(--endspace-text-primary)]'
                />
          ...
          <button
            ...
            className='text-[var(--endspace-text-muted)] hover:text-[var(--endspace-accent-yellow)]'
          >
            <IconX size={18} />
          </button>
        </div>
                ...
                toc.map(t => {
                  ...
                  return (
                    <a
                      key={id}
                      href={`#${id}`}
                      className={`block py-1 text-xs transition-colors rounded px-2 -mx-2 ${isActive ? 'text-[var(--endspace-accent-yellow)] font-bold bg-[var(--endspace-accent-yellow-dim)]' : 'text-[var(--endspace-text-secondary)] hover:text-[var(--endspace-accent-yellow)] hover:bg-[var(--endspace-accent-yellow-dim)]'}`}
                      style={{
      ...
      <div className='fixed right-4 bottom-8 z-50 flex flex-col items-end gap-2 pointer-events-none'>
        {/* Capsule */}
        <div className='bg-white/80 dark:bg-[#3A2830]/80 backdrop-blur-sm p-1.5 rounded-full shadow-lg flex flex-row lg:flex-col gap-3 pointer-events-auto'>
          {showDarkToggle && (
            <ControlBtn
          ...
          {hasToc && (
            <ControlBtn
              ...
              showPercent={true}
              iconClassName='text-[var(--endspace-text-secondary)]'
              iconSize={28}
            />
          )}
